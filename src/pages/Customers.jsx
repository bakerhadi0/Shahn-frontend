...
export default function Customers(){
  ...
  const load=async()=>{const {data}=await http.get("/api/customers");setItems(data);};
  useEffect(()=>{load();},[]);
  const add=async e=>{e.preventDefault();await http.post("/api/customers",f);setF({name:"",company:"",phone:"",location:"",notes:""});load();};
  return(<div><h1>العملاء</h1>
    <form onSubmit={add}>
    ...
    <table><thead><tr><th>الاسم</th><th>الشركة</th><th>الهاتف</th></tr></thead>
      <tbody>{items.map(x=><tr key={x._id}><td>{x.name}</td><td>{x.company}</td><td>{x.phone}</td></tr>)}</tbody>
    </table>
  </div>);
}
